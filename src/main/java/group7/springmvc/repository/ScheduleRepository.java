...
//	List<VaccineSchedule> findAllSchedulesAfterToday(@Param("today") Date today);
	@Query("SELECT vs FROM VaccineSchedule vs WHERE vs.vaccinationDate > :today ORDER BY vs.vaccinationDate DESC")
	List<VaccineSchedule> findAllSchedulesAfterToday(@Param("today") Date today);
	...
	@Query(value = "SELECT v.name AS vaccine_name, " +
	        "CASE " +
	        "WHEN DATEDIFF(YEAR, u.birthday, GETDATE()) BETWEEN 0 AND 9 THEN '0-9' " +
	        "WHEN DATEDIFF(YEAR, u.birthday, GETDATE()) BETWEEN 10 AND 19 THEN '10-19' " +
	        "WHEN DATEDIFF(YEAR, u.birthday, GETDATE()) BETWEEN 20 AND 29 THEN '20-29' " +
	        "WHEN DATEDIFF(YEAR, u.birthday, GETDATE()) BETWEEN 30 AND 39 THEN '30-39' " +
	        "WHEN DATEDIFF(YEAR, u.birthday, GETDATE()) BETWEEN 40 AND 49 THEN '40-49' " +
	        "WHEN DATEDIFF(YEAR, u.birthday, GETDATE()) BETWEEN 50 AND 59 THEN '50-59' " +
	        "ELSE '60+' " +
	        "END AS age_group, " +
	        "COUNT(*) AS patient_count " +
	        "FROM vaccine_schedule vs " +
	        "JOIN vaccine v ON vs.vaccine_id = v.id " +
	        "JOIN patient p ON vs.patient_id = p.id " +
	        "JOIN usermain u ON p.user_id = u.id " +
	        "WHERE v.id = :vaccineId AND vs.status = 'COMPLETED' " +
	        "GROUP BY v.name, " +
	        "CASE " +
	        "WHEN DATEDIFF(YEAR, u.birthday, GETDATE()) BETWEEN 0 AND 9 THEN '0-9' " +
	        "WHEN DATEDIFF(YEAR, u.birthday, GETDATE()) BETWEEN 10 AND 19 THEN '10-19' " +
	...
	@Query("SELECT CAST(v.vaccinationDate AS date) AS date, COUNT(v) AS count " +
		       "FROM VaccineSchedule v " +
		       "WHERE CAST(v.vaccinationDate AS date) = :date " +
		       "GROUP BY CAST(v.vaccinationDate AS date) " +
		       "ORDER BY date")
		List<Object[]> findStatisticsByDay(@Param("date") Date date);


		@Query("SELECT DAY(v.vaccinationDate) AS day, COUNT(v) AS count " +
			       "FROM VaccineSchedule v " +
			       "WHERE YEAR(v.vaccinationDate) = :year " +
			       "AND MONTH(v.vaccinationDate) = :month " +
			       "GROUP BY DAY(v.vaccinationDate) " +
			       "ORDER BY day")
